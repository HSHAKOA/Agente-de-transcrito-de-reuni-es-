...
/** Medidores de nível, transcrição ao vivo e progresso da transcrição --
 * tudo via SSE (Fases C/D). Só faz sentido enquanto há áudio sendo
 * capturado; um reprocessamento não renderiza isto (nem abre os streams). */
          ...
          <Card>
            <dl className="grid grid-cols-3 gap-2 text-center text-sm">
              ...
              <div>
                ...
                <dd className={`mt-0.5 font-mono ${backlog.status === "BEHIND" ? "text-amber-400" : ""}`}>
 ...
 *
 * Também é a tela de um **reprocessamento** (`status.mode === "resume"`):
 * ele ocupa o mesmo slot de processo, mas não captura áudio, então mostra só
 * o cronômetro e o botão de parar.
 *
...
export function Recording({ status }: RecordingProps) {
  ...
  const resuming = status.mode === "resume";
    ...
    <div className="mx-auto max-w-2xl px-4 py-10">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium tracking-wide text-red-400">{resuming ? "REPROCESSANDO" : "GRAVANDO"}</span>
        </div>
      ...
      <h1 className="mt-3 text-lg font-medium">
        {meetingFolderName(status.meeting_dir) || (resuming ? "Sessão interrompida" : "Reunião em andamento")}
      </h1>

      {resuming ? (
        <p className="mt-4 text-sm text-neutral-400">
          Transcrevendo os blocos que ficaram pendentes. Nenhum áudio novo está sendo gravado; o que já foi transcrito
          não é refeito.
        </p>
      ) : (
        <CaptureSections />
      ...
      >
        ...
        {stopping ? "Finalizando…" : resuming ? "Parar reprocessamento" : "Parar reunião"}
      </button>
    </div>
  );
}
